	...
	def update
		...
		if item.save!
			if params[:is_delete]
				...
				if params[:finished]
					log = ActionLog.create(user: @user, todo_list: @list, action: 'finish', content: item.content, logable: item)
				else
					log = ActionLog.create(user: @user, todo_list: @list, action: 'recover', content: item.content, logable: item)
				end
			end			
